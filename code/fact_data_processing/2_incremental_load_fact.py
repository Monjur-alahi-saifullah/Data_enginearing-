...
df_defolt=spark.sql(f"SELECT * FROM {catalog}.{'default'}.{'orders'};")
df_defolt.show(10)
...
df_defolt.write\
 ...
 .option("delta.enableChangeDataFeed", "true") \
 .mode("overwrite") \
 .saveAsTable(f"{catalog}.{bronze_s}.staging_{data_source}")

# COMMAND ----------

row_count_data_source = spark.sql(f"SELECT COUNT(*) AS row_count FROM {catalog}.{bronze_s}.{data_source}").collect()[0]["row_count"]
row_count_staging = spark.sql(f"SELECT COUNT(*) AS row_count FROM {catalog}.{bronze_s}.staging_{data_source}").collect()[0]["row_count"]

print(f"Rows in {data_source}: {row_count_data_source}")
print(f"Rows in staging_{data_source}: {row_count_staging}")
...
df_orders = spark.sql(f"SELECT * FROM {catalog}.{bronze_s}.staging_{data_source};")
df_orders.show(2)
...
silver_table=f"{catalog}.{silver_s}.{data_source}"
...
df_joined.write\
 ...
 .option("delta.enableChangeDataFeed", "true") \
 .mode("overwrite") \
 .saveAsTable(f"{catalog}.{silver_s}.staging_{data_source}")
...
df_gold = spark.sql(f"SELECT order_id, order_placement_date as date, customer_id as customer_code, product_code, product_id, order_qty as sold_quantity FROM {catalog}.{silver_s}.staging_{data_source};")
...
gold_table=f"{catalog}.{gold_s}.sb_dim_{data_source}"
...
df_child =  spark.sql(f"SELECT order_placement_date as date FROM {catalog}.{silver_s}.staging_{data_source}")
...
monthly_table = spark.sql(f"""
    ...
    FROM {catalog}.{gold_s}.sb_dim_orders sbf
...
gold_parent_delta = DeltaTable.forName(spark, f"{catalog}.{gold_s}.fact_orders")
...
# MAGIC %sql
# MAGIC DROP TABLE fmcg.default.orders;
